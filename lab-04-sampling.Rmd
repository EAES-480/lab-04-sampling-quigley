...
> GM GRADE 8.5/10 - Good effort with some good observations on seasonality and daily cycles. However, there were some missing responses (data dictionary) and slight mis-interpretations.
...
> *I chose gpp because it represents plant vigor and should closely respond to season and day-night changes. During winter, it should be pretty clear that there is little vegetation activity, whereas it will be obvious in summer when there is lots of photosynthesis. *

> GM: -0.5 You wrote that you chose `gpp` but then used `ta` (air temperature) in all the code and plots. This inconsistency made several sections confusing.
...
> GM: -0.5 The data dictionary is completely empty. You need to fill in at least 5 variables with units, descriptions, and expected seasonality as instructed.
...
> *There appears to be a clear seasonal cycle where it gets warmer in summer and cools in winter. There was some -9999 daytime values that had to be filtered out. *

> GM: -0.25 You noted the seasonal cycle but the response is incomplete — you should also comment on the daily cycle and any outliers or gaps (you even mentioned filtering -9999 values).
...
> *There appears to be a bimodal distribution with one converging around ~5C and another around 20C. This could be due to these being the average temperatures of the cold and warm seasons. *
...
> *The box plots in the monthly grouping seemed to do better than the day/night grouping. This makes sense since temperature variation can be more extreme between January and July than it would be for day-night cycles. For example, the daytime range was ~60 degrees whereas the monthly ranges where no more than ~40 degrees*
...
> *Choosing a sample size of 10,000, I expected a closer sample mean to the population mean, however my sample mean was still ~0.06 off. It is interesting that the sample mean tends to underestimate the population mean. *

> GM: -0.25 With a large sample size (n=1000) the sample mean should be very close to the population mean. A difference of 0.06 is small and expected due to sampling variability.
...
> *the sampling distribution looks like it's close to the population mean. As n_samp increases, the closer the sample mean becomes to the population mean and the distribution begins to look more normal. *
---
...
> *Stratifying by month seemed to be a better way to get the mean and sd for my response variable because the most important factor in air temperature is season. Additionally, there was less variance in the summer months whereas the winter months tended to have more outliers, so stratifying by month may help reduce the effectiveness of these outliers. The random sampling could over select samples from certain seasons or months, which would skew the mean and SD. * 
...
> *Seasonality is important for sampling because for environmental datasets, the season can have large effects on the values of variables, therefore seasonality must be controlled for when doing analysis. If seasonality is ignored, this can result in unrepresentative samples. Stratification is therefore essential in EAES fields where ecologic, hydrologic, and atmospheric systems can all be greatly affected by systems. The issue with stratifying is it requires previous knowledge to stratify in a useful manner. For example, temperature can be controlled for by stratifying by month since we know that temperatures and month fluctuate together. *
...
k <- 24 # every 12 hours to capture diurnal cycle
...
> *My cycle is diurnal to capture daytime and nighttime conditions. the sample mean was acutally futher away to the population mean than the SRS. This is surprising to me as I thought capturing the temperature twice daily would be pretty close to the mean, however, when I tried with either 6 or 96 hours, the mean was much closer. *

> GM: Systematic sampling interacted with the daily cycle depending on your choice of `k`. This is a common issue with time series data.
...
> *The cluster sampling can have higher variance than SRS because across several days, the min and max values can change significantly. Observations from within a single day may be more likely to be correlated to one another (autocorrelation) rather than them being independent measurements. By sampling entire weeks, it may capture a wider range of the temperature values across time. *

> GM: Solid explanation — autocorrelation within days reduces independent information and can increase variance.
...
> *Using quasi-continuous sampling, the seasonal and daily patterns were preserved with a little smoothing from the raw data. Since I chose hourly sampling, it definitely preserved the diurnal cycle. The larger the interval, the smoother the data, which may make long term patterns more obvious. This means in EAES studies looking at gradual changes, longer term quasi-continuous sampling may be perferable. *
...
> *It appears that quasi-continuous provided the closest mean and SD with the lowest error. This may be because it sampled at regular hourly intervals, which is likely to capture temperature changes well and additionally it had the highest number of samples. Cluster sampling performed poorly due to days being sampled together and this reduced the sample size to only 334. Systematic sampling also introduced a lot of error because then interval I chose interacted with the diurnal cycle, missing portions of the diurnal cycle. If I were designing an EAES study about annual atmospheric temperature with limited field days, I would try and stratify across seasons (ie sampling days within each month), which would optimize for seasonal representation while reducing the total number of days in the field. This approach however, would likely miss diurnal variation. *
